...
public class LaprasControl extends Component {
    ...
    public LaprasControl() {
        ...
        for (int i = 1; i <= 2; i++) {
            runFrames.add(FXGL.image("lapras/idle/spawn" + i + ".png", 43*playerScale, 34*playerScale));
        }
        ...
        runFrames.clear();
        for (int i = 1; i <= 3; i++) {
            runFrames.add(FXGL.image("lapras/movimento/movimentolaterale/movimentolaterale" + i + ".png", 32*playerScale, 39*playerScale));
        }
        animRunSide = new AnimationChannel(runFrames, Duration.seconds(0.7));

        runFrames.clear();
        for (int i = 1; i <= 3; i++) {
            runFrames.add(FXGL.image("lapras/movimento/movimentosu/movimentosu" + i + ".png", 43*playerScale, 40*playerScale));
        }
        animRunUp = new AnimationChannel(runFrames, Duration.seconds(0.7));

        runFrames.clear();
        for (int i = 1; i <= 3; i++) {
            runFrames.add(FXGL.image("lapras/movimento/movimentogiu/movimentogiu" + i + ".png", 43*playerScale, 34*playerScale));
        }
        animRunDown = new AnimationChannel(runFrames, Duration.seconds(0.7));

        runFrames.clear();
        for (int i = 1; i <= 6; i++) {
            runFrames.add(FXGL.image("lapras/morte/morte" + i + ".png", 35*playerScale, 42*playerScale));
        }
        animDeath = new AnimationChannel(runFrames, Duration.seconds(1.5));
    ...
    public void onUpdate(double tpf) {
        if(entity.getComponent(VitaComponent.class).isDead()){
            ...
            if(texture.getAnimationChannel() != animDeath){
                getNotificationService().pushNotification("HAI UCCISO LAPRAS");
                entity.getComponent(VitaComponent.class).muori(texture, animDeath);
            }
        }

        //MOVIMENTO AUTOMATICO DI LAPRAS
        if (FXGL.getGameWorld().getEntitiesByType(PokemonTypes.PLAYER).getFirst().getCenter().distance(FXGL.getGameWorld().getEntitiesByType(PokemonTypes.RINGLAPRAS).getFirst().getCenter()) <= FXGL.getGameWorld().getEntitiesByType(PokemonTypes.RINGLAPRAS).getFirst().getWidth()/2 && !entity.getComponent(VitaComponent.class).isDead()){
            //ASSE X
